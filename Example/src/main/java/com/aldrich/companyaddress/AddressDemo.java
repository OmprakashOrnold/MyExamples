													...
													if (!address.toLowerCase().contains("telephone:")) {
														if (!address.toLowerCase().contains("(toll free)")
															...
															if (isNumeric == false) {
																...
																/*String adText=StringUtil.removeSpecialCharacters(new_address) ;
																Map<AddressComponent, String> parsedAddr  = AddressParser.parseAddress(adText);
																if(AddressStandardizer.normalizeParsedAddress(parsedAddr)!=null)
																{
