...
public class IndeedWebsiteLinks {
	public static void main(String[] args){
		...
		while (it.hasNext()) {
			...
			try {
				doc=Jsoup.connect(urlLinks).followRedirects(true).ignoreHttpErrors(true).get();
				if (doc!=null) {
					Elements firstLink=doc.getElementsByClass("icl-u-lg-mr--sm icl-u-xs-mr--xs");
						...
						if(doc1!=null) { 
							...
							if(object.has("aboutStory")) 
							{
								...
								if(obj.has("aboutMetadata")) {
									...
									Elements secondLink=doc.getElementsByClass("icl-u-lg-inline icl-us-xs-hide");
									if (secondLink!=null && !secondLink.isEmpty()) {
										for (Element element : secondLink) {
											try {
												redirecturl=element.tagName("a").children().attr("href");
												Response response = Jsoup.connect(redirecturl).ignoreHttpErrors(true).execute();
