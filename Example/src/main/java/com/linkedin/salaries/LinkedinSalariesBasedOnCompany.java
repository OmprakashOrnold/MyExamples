...
public class LinkedinSalariesBasedOnCompany {

	public static void main(String[] args) {

		Document document = null;

		File folder = new File("C:\\OmPrakash\\Linkedin Salaray");
		File[] listOfFiles = folder.listFiles();
		for (File file : listOfFiles) {
			if (file.isFile()) 
			{
				String fullFileName="C:\\OmPrakash\\Linkedin Salaray\\"+file.getName();
				...
				try {
					...
					if (elements!=null && elements.size()>0) {
						for (Element element : elements) {
							...
							Elements companyNameElement = element.getElementsByClass("title");
							if (companyNameElement != null && companyNameElement.size() > 0) {

								companyName=companyNameElement.text().replace(designationTitle, "").replace("salaries", "").trim();						
							}
							...
							String outputFile = "C:\\Om\\linkedin_salary_by_company.csv";
